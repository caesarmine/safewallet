...
module.exports = (shepherd) => {
  ...
  shepherd.post('/encryptkey', (req, res, next) => {
    if (req.body.key &&
      ...
      fs.writeFile(`${shepherd.safewalletDir}/shepherd/pin/${req.body.pubkey}.pin`, encryptedString, (err) => {
        if (err) {
  ...
  shepherd.post('/decryptkey', (req, res, next) => {
    if (req.body.key &&
        req.body.pubkey) {
      if (fs.existsSync(`${shepherd.safewalletDir}/shepherd/pin/${req.body.pubkey}.pin`)) {
        fs.readFile(`${shepherd.safewalletDir}/shepherd/pin/${req.body.pubkey}.pin`, 'utf8', (err, data) => {
          if (err) {
            const errorObj = {
              msg: 'error',
              result: err,
