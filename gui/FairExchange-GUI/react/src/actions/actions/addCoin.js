...
export function activeHandle() {
  return dispatch => {
    return fetch(`http://127.0.0.1:${Config.safewalletPort}/shepherd/auth/status`, {
      method: 'GET',
      headers: {
...
export function shepherdElectrumAddCoin(coin) {
  return dispatch => {
    return fetch(`http://127.0.0.1:${Config.safewalletPort}/shepherd/electrum/coins/add?coin=${coin}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      }
...
export function _shepherdGetConfig(coin, mode, startupParams) {
  return dispatch => {
    return fetch(`http://127.0.0.1:${Config.safewalletPort}/shepherd/getconf`, {
      ...
      body: JSON.stringify({ chain: 'safecoind' })
    })
    .catch((error) => {
      console.log(error);
      dispatch(
